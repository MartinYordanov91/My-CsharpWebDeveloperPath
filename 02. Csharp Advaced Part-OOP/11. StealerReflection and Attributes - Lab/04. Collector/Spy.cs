...
namespace Stealer
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string className)
        {
            StringBuilder sb = new();
            Type type = Type.GetType(className);
            MethodInfo[] methodInfosGetAndSet = type.GetMethods((BindingFlags)60);

            foreach (MethodInfo item in methodInfosGetAndSet.Where(x=>x.Name.StartsWith("get")))
            {
                sb.AppendLine($"{item.Name} will return {item.ReturnType}");
            }

            foreach (MethodInfo item in methodInfosGetAndSet.Where(x => x.Name.StartsWith("set")))
            {
